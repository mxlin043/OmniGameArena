#!/usr/bin/env bash
# midline_clash (pvp) cold-start -- pdq
# config: configs/vlm/cold_start/pvp/midline_clash/vanilla_pdq.yaml
...
# Deploy the target Qwen model, obtain its host and port, then update the pvp YAML players before uncommenting.
...
GAME=midline_clash
OUTROOT=runs/pdq
CONFIG="configs/vlm/cold_start/pvp/$GAME/vanilla_pdq.yaml"
