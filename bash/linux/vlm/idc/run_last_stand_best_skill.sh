#!/usr/bin/env bash
# last_stand held-out variants with each model's best measured IDC skill.
# Run this after the corresponding IDC runs have produced round results.
...
IDC_ROOT=runs/idc
# Empty = auto-select latest run under IDC_ROOT/last_stand/<model>.
# Set this to one exact IDC run directory for reproducible variant eval.
# Example: IDC_RUN=runs/idc/last_stand/claude-opus-4-7/20260530_120000
IDC_RUN=
...
CONFIG_PATTERN="configs/vlm/cold_start/solo/last_stand/variant_pdq_{variant}.yaml"
...
if [ ! -f "configs/vlm/cold_start/solo/last_stand/variant_pdq_var1.yaml" ]; then echo "[error] variant config not found for $GAME"; exit 2; fi
