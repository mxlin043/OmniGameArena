...
@REM monster_shoot (solo) cold-start -- lcrt
@REM config: configs\vlm\cold_start\solo\monster_shoot\vanilla_lcrt.yaml
...
set "GAME=monster_shoot"
set "OUTROOT=runs\lcrt"
set "CONFIG=configs\vlm\cold_start\solo\%GAME%\vanilla_lcrt.yaml"
cd /d "%~dp0..\..\..\..\.."
if not exist "%CONFIG%" ( echo [error] config not found: %CONFIG% & exit /b 2 )

echo ===== %GAME% / lcrt / %IP%:%PORT% target=%EPISODES% count=%COUNT% =====
for %%M in (!MODELS!) do (
