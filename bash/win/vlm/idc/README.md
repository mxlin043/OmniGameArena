# VLM IDC Windows Launchers

This directory contains Windows `.cmd` launchers for VLM Improvement Dynamics Curve (IDC) runs and the follow-up best-skill variant evaluation.

Run these scripts from Command Prompt, or double-click after editing the `TWEAK HERE` block. Each script changes to the repository root before invoking Python.

## Scripts

| Script | Purpose |
| --- | --- |
| `run_last_stand.cmd` | Run IDC on the LastStand base map. |
| `run_shared_floor.cmd` | Run IDC on the SharedFloor coop base map. Player 1 and player 2 use the same model and shared skill. |
| `run_last_stand_best_skill.cmd` | Evaluate LastStand held-out variants with the best measured IDC skill. |
| `run_shared_floor_best_skill.cmd` | Evaluate SharedFloor held-out variants with the best measured IDC skill. |
...
```cmd
bash\win\vlm\idc\run_last_stand.cmd
bash\win\vlm\idc\run_shared_floor.cmd
```
...
| Variable | Meaning |
| --- | --- |
| `MODEL` | Player model for the IDC run. The default active model is `claude-opus-4-6`; other examples are commented with `@REM`. |
...
```cmd
set "RESUME=runs\idc\last_stand\claude-opus-4-6\20260529_191934"
```
...
```cmd
bash\win\vlm\idc\run_last_stand_best_skill.cmd
bash\win\vlm\idc\run_shared_floor_best_skill.cmd
```
...
```cmd
set "IDC_RUN=runs\idc\last_stand\claude-opus-4-6\20260529_191934"
```
